...
temp_plot = ax.pcolormesh(lon_grid, lat_grid, temps[0], transform=ccrs.PlateCarree(), cmap='coolwarm')
...
def update_plot(frame):
    ax.coastlines()
    ax.gridlines()
    temp_plot = ax.pcolormesh(lon_grid, lat_grid, temps[frame, :, :], transform=ccrs.PlateCarree(), cmap='coolwarm')
    #cbar = plt.colorbar(temp_plot, orientation='vertical', pad=0.05, aspect=50)
    #cbar.set_label('Land Surface Temperature (K)')
